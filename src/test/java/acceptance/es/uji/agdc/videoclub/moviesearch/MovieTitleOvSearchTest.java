...
@Transactional
public class MovieTitleOvSearchTest {
    ...
    @Test
    public void findAll_withTitleOv_returnsAllFullAndPartialMatchingMoviesOrdered() throws Exception {
        // Given two different movies on the database that have a similar title in original version
        service.create(movie);
        service.create(anotherMovie);

        // When the system performs a query to search them
        Stream<Movie> movies = service.findAllBy(MovieQueryTypeMultiple.TITLE_OV, "Captain America");
    ...
    @Test
    public void findAll_withNoTitleOv_returnsEmptyListOfMovies() throws Exception {
        // Given two different movies on the database that have a similar title in original version
        service.create(movie);
        service.create(anotherMovie);
